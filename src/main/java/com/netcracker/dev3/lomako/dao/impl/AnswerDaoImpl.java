...
public enum  AnswerDaoImpl implements AnswerDao {
    ...
    private static final String SELECT_ANSWER_BY_TASK_ID_SQL =
            "SELECT id, `order`, text, is_correct, tasks_id FROM answers WHERE tasks_id = ?";
    ...
    public List<Answer> findByTaskId(long taskId) {
        ...
        try(Connection connection = connectionPool.getConnection()) {
            PreparedStatement selectAll = connection.prepareStatement(SELECT_ANSWER_BY_TASK_ID_SQL);
            selectAll.setLong(1, taskId);
