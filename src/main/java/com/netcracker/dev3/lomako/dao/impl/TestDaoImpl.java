...
public enum  TestDaoImpl implements TestDao {
    ...
    private static final String SELECT_BY_USER_ID_SQL =
            "SELECT id, name, description, user_id, strategy_id, created, updated FROM tests WHERE user_id = ?";
    ...
    public List<Test> findByUserId(long userId) throws SQLException {
        ...
        try(Connection connection = connectionPool.getConnection()) {
            PreparedStatement selectByUserId = connection.prepareStatement(SELECT_BY_USER_ID_SQL);
            selectByUserId.setLong(1, userId);

            ResultSet resultSet = selectByUserId.executeQuery();
